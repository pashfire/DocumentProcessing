    ...
    resolutionPageVueInstance = new Vue({
        ...
        methods: {
            ...
            saveDocument() {
                ...
                if (this.inspectSwitch) {
                    formData.append("ControllerNote", 'Контроль ' + regDate.toLocaleDateString());
                    //this.ControllerNote = 'Контроль ' + regDate.toLocaleDateString();
                } else formData.append("ControllerNote", this.form.ControllerNote);

                Vue.http.post('/Document/Inspect/', formData)
                    .then(response => {
                        //success callback
